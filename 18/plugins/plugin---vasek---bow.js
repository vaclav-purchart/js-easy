...
VoxelWorld.registerPlugin('Bow', {
	async init(api) {
		...
		function drawBowSprite(ctx, W, H, drawn) {
			...
			// Bow stave — pixel-art arc along the left side
			ctx.fillStyle = '#7B3F00'
			const stave = [
				[5,0],[4,1],[3,2],[3,3],[2,4],[2,5],[2,6],
				[2,7],[2,8],[2,9],[2,10],[3,11],[3,12],[4,13],[5,14],[5,15],
			]
			for (const [x, y] of stave) ctx.fillRect(x, y, 1, 1)

			// Bowstring — straight when idle, pulled back when drawn
			ctx.fillStyle = '#E8D8A0'
			const pull = drawn ? 4 : 0
			for (let y = 0; y <= 15; y++) {
				// Fraction 0 at middle (y=7.5), 1 at tips
				const frac = Math.abs(y - 7.5) / 7.5
				const x = Math.round(5 + pull * (1 - frac))
				ctx.fillRect(x, y, 1, 1)
			}

			// Nocked arrow only when drawn
			if (drawn) {
		...
		function makeArrowMesh() {
			...
			// Two crossed feather planes at the tail
		...
		function launchArrowVisual(startPos, direction, targetPos) {
			...
			// Orient the arrow group so its +Z faces the direction of travel.
		...
		api.registerTool({
			...
			onRightUp(ctx) {
				...
				// Start the arrow at eye level, 0.3 blocks in front of camera
				const camDir = new THREE.Vector3()
				api.camera.getWorldDirection(camDir)
				const startPos = api.camera.position.clone().addScaledVector(camDir, 0.3)
				...
				if (hit) {
					...
					if (hit.type === 'mob') {
						api.netSend({ type: 'hit_mob', mobId: hit.id, damage })
					} else if (hit.type === 'player') {
						api.netSend({ type: 'hit_player', targetId: hit.id, damage })
					}
